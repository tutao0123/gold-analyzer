...
def fetch_event_description(query: str) -> str:
    """
    使用百炼 qwen-plus 的联网能力，根据查询总结历史金融事件对黄金的影响。
    """
    api_key = os.getenv("DASHSCOPE_API_KEY")
    client = OpenAI(api_key=api_key, base_url="https://dashscope.aliyuncs.com/compatible-mode/v1")
    
    print(f"   [LLM联网生成中] 查阅事件新闻: {query}")
    try:
        ...
        print(f"   [警告] 大模型描述生成失败: {e}")
        return "本地备用空描述：获取新闻摘要失败。"


def generate_kline_image(symbol="GC=F", start_date=None, end_date=None, output_path=None):
    """
    使用雅虎财经下载历史数据并绘制成 PNG 图像
    """
    print(f"   [{start_date} to {end_date}] 正在下载... ")
...
def build_historical_knowledge():
    """
    构建历史重大行情的知识库 (包含 LLM 新闻补全能力)
    """
    print("=== 开始构建金价历史图像多模态知识库 (联网全自动化版) ===")
    
    rag = GoldMultimodalRAG()
    if not rag.api_key:
        print("缺少 DASHSCOPE_API_KEY，脚本无法执行。")
        return
