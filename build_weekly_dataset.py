...
import platform
from gold_rag import GoldMultimodalRAG
...
async def main(years=5):
    ...
    img_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "kline_weekly_images")
    os.makedirs(img_dir, exist_ok=True)
    
    print(f"正在从 Yahoo Finance 下载近 {years} 年的金价（GC=F）日线数据...")
    df = yf.download("GC=F", period=f"{years}y")
    if df.empty:
        print("下载数据为空。")
        return
